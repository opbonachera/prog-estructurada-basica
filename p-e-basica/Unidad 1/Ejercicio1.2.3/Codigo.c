...
int main(){
    ...
    CantAlumnos = Ingreso(A,80);
    Resultado(A,CantAlumnos);
    Promocionados(A, CantAlumnos);
...
int Ingreso(struct ALUMNO A[], int CantMax){
    ...
    while((DNI!=0 && i<CantMax)){
        ...
        A[i].NPromedio=(float) (A[i].Nota1 + A[i].Nota2) / 2;
...
void Resultado(struct ALUMNO A[], int Cant){
    int ContadorAprobados=0, ContadorDesaprobados=0, i=0;
...
void Promocionados(struct ALUMNO A[], int Cant){
    ...
    for(i=0;i<Cant;i++){
        if(A[i].Nota1 >7 && A[i].Nota2>7){
            printf("   %s        %d         %f ",A[i].NombreApellido, A[i].NumeroDNI, A[i].NPromedio);
    }
    }
}
